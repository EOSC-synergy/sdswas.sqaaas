...
def ret_scores(mod_df, obs_df, grp=None, grpname=None, obs='aeronet'):
    ...
    if mvar not in tot.columns:
        mvar = mvar.lower()
    print('TOT1', tot[mvar], 'TOT2', tot[ovar])
    try:
...
def convert2timeseries(model, obs=None, months=None):
    ...
    try:
        ...
        if 'sites' in OBS[obs]:
            ...
            print("Station re-numbering")
            # re-numbering stations
            for old_st, new_st in zip(old_stations, new_stations):
                obs_df.loc[obs_df['station'] == old_st, 'station'] = new_st

            print("Station adding areas")
    ...
    for mod_idx, mod_ds in enumerate(models_ds):
        ...
        frge = []
        totn = []
        print('IDX', mod_idx, list(model)[mod_idx])
        if mod_ds is None:
            print('MOD is NONE!!! Continue')
            bias_df['station'] = ''
            bias_df[columns[mod_idx+1]] = '-'
            rmse_df['station'] = ''
            rmse_df[columns[mod_idx+1]] = '-'
            corr_df['station'] = ''
            corr_df[columns[mod_idx+1]] = '-'
            frge_df['station'] = ''
            frge_df[columns[mod_idx+1]] = '-'
            totn_df['station'] = ''
            totn_df[columns[mod_idx+1]] = 0
            continue
        for var in mod_ds.variables:
            ...
            if obs_ds:
                ...
                try:
                    mod_interp = mod_ds[curvar].interp(longitude=obs_lon, latitude=obs_lat)
                except:
                    mod_interp = mod_ds[curvar].interp(lon=obs_lon, lat=obs_lat)
                print('converting to df  ...')
            else:
                mod_interp = mod_ds[curvar]
